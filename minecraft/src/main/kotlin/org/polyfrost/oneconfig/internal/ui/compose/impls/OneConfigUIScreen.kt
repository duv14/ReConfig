...
import org.polyfrost.oneconfig.internal.ui.navigation.ReConfigRouteMemory
import org.polyfrost.oneconfig.internal.ui.sound.UiSoundEvent
import org.polyfrost.oneconfig.internal.ui.sound.UiSounds
import org.polyfrost.oneconfig.internal.reconfig.KeyCapture
...
class OneConfigUIScreen @JvmOverloads constructor(
    ...
    companion object {
        ...
        private fun resolveOpeningBehaviorRoute(): OpeningRoute =
            OpeningRoute(ReConfigRouteMemory.routeForRememberedPage(), restored = true)

        private fun resolveRoute(): OpeningRoute = when (OneConfigConfig.openingBehavior) {
            0 -> OpeningRoute(FriendsGraph)
            1 -> OpeningRoute(PreferencesGraph)
            2 -> OpeningRoute(FriendsGraph)
            3 -> {
                ...
                if (withinWindow && route != null) OpeningRoute(route, restored = true) else OpeningRoute(FriendsGraph)
            }
            else -> OpeningRoute(FriendsGraph)
        }
    ...
    private fun markClosed() {
        ...
        ReConfigRouteMemory.rememberPage(ShellState.lastRoute, ShellState.lastClosedAt)
    }
    ...
    override fun init() {
        ...
        try {
            ...
            ShellState.playerName = mc.user.name
            ShellState.playerUuid = runCatching {
                val user = mc.user
                val method = user.javaClass.methods.firstOrNull { it.name in setOf("getProfileId", "profileId") && it.parameterCount == 0 }
                method?.invoke(user)?.toString()?.replace("-", "") ?: ""
            }.getOrDefault("")
            ShellState.serverAddress = mc.currentServer?.ip?.lowercase()?.trim().orEmpty()
        } catch (_: Throwable) {
        ...
        try {
            ...
            ShellState.versionLabel = "ReConfig"
        }
    ...
    override fun handleKeyPressed(key: Int, modifiers: Int): Boolean {
        if (KeyCapture.moduleId != null && KeyCapture.consume(key, 1)) return true
    ...
    override fun compose() {
        val initialRoute = route ?: FriendsGraph
