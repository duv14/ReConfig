...
abstract class ComposeScreen(
    ...
    override fun init() {
        ...
        if (!bindContent()) {
            closeWithMessage("ReConfig's UI failed to start. Please check your logs and report this.")
        }
    }
    ...
    private fun reportUnavailableAndClose() {
        val reason = ComposeSupport.unavailableReason()
            ?: "ReConfig's UI failed to start. Please check your logs and report this."
    ...
    override fun extractRenderState(ctx: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, tickDelta: Float) {
        ...
        if (scenePoisoned) {
            if (sceneRebuilds >= MAX_SCENE_REBUILDS) {
                ...
                closeSceneQuietly()
                closeWithMessage("ReConfig's UI hit a rendering error and had to close.")
                return
            }
            sceneRebuilds++
            onSceneRebuilding()
            ...
            if (rebuilt == null || !bindContent()) {
                ...
                closeWithMessage(
                    ComposeSupport.unavailableReason()
                        ?: "ReConfig's UI hit a rendering error and had to close."
                )
                return
            }
            sceneDirty = true
