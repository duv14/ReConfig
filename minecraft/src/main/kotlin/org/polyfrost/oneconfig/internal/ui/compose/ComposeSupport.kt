...
object ComposeSupport {
    ...
    fun recordSceneFailure(error: Throwable) {
        ...
        LOG.error("Compose is unavailable on this system; the ReConfig UI has been disabled.", error)
    }
        ...
        while (e != null && seen.add(e)) {
            ...
            when {
                e is UnsatisfiedLinkError || "UnsatisfiedLinkError" in message ->
                    return "ReConfig's UI can't start: the Skia native library failed to load for " +
                        "$osLabel. This platform isn't supported by ReConfig's renderer."

                "java.awt" in message || "Toolkit" in message || "libawt" in message ->
                    return "ReConfig's UI can't start: this Java runtime has no working AWT support " +
    ...
    private fun probe(): String? {
        if (System.getProperty(DISABLE_PROPERTY) != null) {
            return "ReConfig's UI is disabled by the -D$DISABLE_PROPERTY JVM flag."
        }
        if (!classExists("java.awt.Toolkit") || !classExists("org.jetbrains.skiko.Library")) {
            return "ReConfig's UI can't start on $osLabel: a required library (AWT or Skiko) is " +
    ...
    private fun awtInitFailure(): String? = try {
        ...
        LOG.error("AWT failed to initialize on this runtime; the ReConfig UI has been disabled.", t)
        "ReConfig's UI can't start on $osLabel: this Java runtime has no working AWT support " +
