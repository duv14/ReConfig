...
export default { async fetch(request, env) {
  ...
  try {
    ...
    if(path==="/v2/messages"&&request.method==="POST"){const b=await request.json(),other=await findUser(env,b.to),body=String(b.body||"").trim().slice(0,500);if(!other||!body)return json({error:"invalid message"},400);if(!(await isFriend(env,me.uuid,other.uuid)))return json({error:"friendship required"},403);const result=await env.DB.prepare("INSERT INTO messages_v2(sender,recipient,body,created_at) VALUES(?,?,?,?)").bind(me.uuid,other.uuid,body,Date.now()).run();return json({ok:true,id:Number(result.meta?.last_row_id||result.lastInsertRowid||0)});}
    ...
    if(path==="/v2/messages"&&request.method==="GET"){const other=await findUser(env,url.searchParams.get("with")),after=Math.max(0,Number(url.searchParams.get("after")||0));if(!other)return json({error:"player not found"},404);const rows=await env.DB.prepare("SELECT id,sender,recipient,body,created_at,acknowledged FROM messages_v2 WHERE id>? AND ((sender=? AND recipient=?) OR (sender=? AND recipient=?)) ORDER BY id LIMIT 200").bind(after,me.uuid,other.uuid,other.uuid,me.uuid).all();return json({messages:rows.results});}
    if(path==="/v2/invitations"&&request.method==="POST"){const b=await request.json(),other=await findUser(env,b.to),address=String(b.serverAddress||"").trim().slice(0,255);if(!other||!address)return json({error:"invalid invitation"},400);if(!(await isFriend(env,me.uuid,other.uuid)))return json({error:"friendship required"},403);await env.DB.prepare("INSERT INTO invitations(sender,recipient,server_address,created_at) VALUES(?,?,?,?)").bind(me.uuid,other.uuid,address,Date.now()).run();return json({ok:true});}
    if(path==="/v2/events"&&request.method==="GET"){const invitations=await env.DB.prepare("SELECT i.id,u.name sender,i.server_address serverAddress,i.created_at FROM invitations i JOIN users u ON u.uuid=i.sender WHERE i.recipient=? AND i.acknowledged=0 ORDER BY i.id LIMIT 50").bind(me.uuid).all();if(invitations.results.length) await env.DB.prepare(`UPDATE invitations SET acknowledged=1 WHERE id IN (${invitations.results.map(()=>"?").join(",")})`).bind(...invitations.results.map(x=>x.id)).run();return json({invitations:invitations.results});}
